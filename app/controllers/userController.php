<?php
	...
	class userController extends mainModel{
		...
		public function registrarUsuarioControlador(){
            ...
            # Verificando usuario #
		    $check_usuario=$this->ejecutarConsulta("SELECT Nombre_Usuario FROM usuario WHERE Nombre_Usuario='$usuario'");
		...
		public function listarUsuarioControlador($pagina,$registros,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT * FROM usuario WHERE ((ID_Usuario!='".$_SESSION['id']."' AND ID_Usuario!='1') AND (Nombre LIKE '%$busqueda%' OR Apellido LIKE '%$busqueda%' OR Email LIKE '%$busqueda%' OR Nombre_Usuario LIKE '%$busqueda%')) ORDER BY Nombre ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(ID_Usuario) FROM usuario WHERE ((ID_Usuario!='".$_SESSION['id']."' AND ID_Usuario!='1') AND (Nombre LIKE '%$busqueda%' OR Apellido LIKE '%$busqueda%' OR Email LIKE '%$busqueda%' OR Nombre_Usuario LIKE '%$busqueda%'))";

			}else{

				$consulta_datos="SELECT * FROM usuario WHERE ID_Usuario!='".$_SESSION['id']."' AND ID_Usuario!='1' ORDER BY Apellido ASC LIMIT $inicio,$registros";
				...
				foreach($datos as $rows){
					$tabla.='
						<tr class="has-text-centered" >
							...
							<td>
			                    <a href="'.APP_URL.'userPhoto/'.$rows['ID_Usuario'].'/" class="button is-info is-rounded is-small">Foto</a>
			                </td>
			                <td>
		...
		public function actualizarUsuarioControlador(){
		    ...
		    $check_admin=$this->ejecutarConsulta("SELECT * FROM usuario WHERE Nombre_Usuario='$admin_usuario' AND ID_Usuario='".$_SESSION['id']."'");
		    if($check_admin->rowCount()==1){

		    	$check_admin=$check_admin->fetch();

		    	if($check_admin['Nombre_Usuario']!=$admin_usuario || !password_verify($admin_clave,$check_admin['Clave'])){
